...
function parseData() {
  for (let i in shardData) {
    ...
    mates.push({
      name: user.Name,
      payout: parseInt(
        user.UTC.substr(0, 2) + user.UTC.substr(-2, user.UTC.length)
      ),
      po: {
...
async function sendMessage() {
  ...
  let desc = "**Payout Countdown Timer**:";
  for (let i in mates) {
    let fieldName = String(mates[i].time) + " hrs until payout";
    let fieldText = "";
  ...
  embed.setFooter("Last refresh", "https://i.imgur.com/WMp4KQy.png");
  embed.setTimestamp();
